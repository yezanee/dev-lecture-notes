...
쉽게 말해, 스프링 빈이 만들어진 직후에 "준비 작업"을 하고, 소멸되기 직전에 "정리 작업"을 할 수 있도록 스프링이 알려주는 것이 생명주기 콜백이다.
...
생성자 부분을 보면 url 정보 없이 `connect`가 호출되는 것을 확인할 수 있다. 너무 당연한 이야기이지만 객체를 생성하는 단계에는 url이 없고, 객체를 생성한 다음에 외부에서 수정자 주입을 통해서 `setUrl()`이 호출되어야 url이 존재하게 된다.
...
- 출력 결과
  ...
  13:24:49.043 [main] DEBUG ... Closing NetworkClient.destroy
...
- 결과
  ...
  connect: http://hello-spring.dev
  call: http://hello-spring.dev message = 초기화 연결 메시지
  13:33:10.029 [main] DEBUG ... Closing NetworkClient.close
  close + http://hello-spring.dev
  ```
...
- 실행 결과
  ...
  connect: http://hello-spring.dev
  call: http://hello-spring.dev message = 초기화 연결 메시지
  19:40:50.269 [main] DEBUG ... Closing NetworkClient.close
  close + http://hello-spring.dev
  ```
...
- 스프링 빈은 **객체 생성 → 의존관계 주입 → 초기화 콜백 → 사용 → 소멸전 콜백 → 종료**의 라이프사이클을 가진다. 초기화 작업(DB 연결, 네트워크 소켓 등)은 의존관계 주입이 완료된 후에 해야 하므로, 생성자가 아닌 콜백 시점에 수행해야 한다.
- 생명주기 콜백은 3가지 방법(인터페이스, `@Bean` 속성, 애노테이션)이 있지만, **`@PostConstruct`와 `@PreDestroy` 애노테이션을 사용하자.** 자바 표준이라 스프링에 종속되지 않고, 애노테이션 하나만 붙이면 되므로 가장 편리하다.
- 외부 라이브러리처럼 코드를 수정할 수 없는 경우에만 `@Bean(initMethod, destroyMethod)`를 사용하면 된다. `destroyMethod`는 기본값이 추론 모드로, `close`나 `shutdown` 이름의 메서드를 자동으로 찾아 호출해준다.
