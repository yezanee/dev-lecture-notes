...
쉽게 말해, 싱글톤은 "컨테이너가 살아있는 동안 계속 존재하는 빈"이고, 프로토타입은 "요청할 때마다 새로 만들어주고 버리는 빈"이다.
...
프로토타입 빈을 직접 스프링 컨테이너에 요청하면 문제없다. 클라이언트A가 요청하면 새로운 빈(x01, count=0)이 생성되고, 클라이언트B가 요청하면 또 새로운 빈(x02, count=0)이 생성된다. 각자 `addCount()`를 호출해도 서로 다른 빈이므로 각각 count가 1이 된다.

#### 싱글톤 빈에서 프로토타입 빈 사용

문제는 `clientBean`이라는 싱글톤 빈이 의존관계 주입을 통해서 프로토타입 빈을 주입받아서 사용하는 경우다.
...
- 클라이언트 A가 `clientBean.logic()`을 호출하면 `prototypeBean`의 `addCount()`가 호출되어 count가 1이 된다.
...
- 클라이언트 B가 `clientBean.logic()`을 호출하면 같은 프로토타입 빈의 `addCount()`가 호출되어 count가 2가 된다.
...
마지막 방법은 `javax.inject.Provider`라는 JSR-330 자바 표준을 사용하는 방법이다. (스프링 부트 3.0은 `jakarta.inject.Provider` 사용)
...
- request scope를 사용하지 않고 파라미터로 이 모든 정보를 서비스 계층에 넘긴다면, 파라미터가 많아서 지저분해진다. 서비스 계층은 웹 기술에 종속되지 않고, 가급적 순수하게 유지하는 것이 유지보수 관점에서 좋다.
...
첫번째 해결방안은 앞서 배운 Provider를 사용하는 것이다. `ObjectProvider`를 사용해보자.
...
이제 나머지 코드를 Provider 사용 이전으로 돌려두자. (`ObjectProvider` 없이 `MyLogger`를 직접 주입받는 원래 코드로)
...
- 빈 스코프는 빈이 존재할 수 있는 범위를 의미한다. 싱글톤(기본)은 컨테이너와 생명주기가 같고, 프로토타입은 생성·주입·초기화까지만 컨테이너가 관여하고 이후 관리하지 않는다.
- 싱글톤 빈이 프로토타입 빈을 주입받으면 프로토타입 빈이 싱글톤처럼 계속 유지되는 문제가 발생한다. 이때 `ObjectProvider`나 `JSR-330 Provider`를 사용하면 사용할 때마다 새로운 프로토타입 빈을 생성할 수 있다. (DL - Dependency Lookup)
- 웹 스코프(request 등)는 HTTP 요청 당 하나의 빈이 생성·관리된다. 직접 주입하면 애플리케이션 시작 시점에 오류가 발생하므로, `ObjectProvider`로 빈 생성을 지연하거나, `proxyMode = ScopedProxyMode.TARGET_CLASS`로 CGLIB 가짜 프록시를 주입해서 해결한다. 핵심 아이디어는 진짜 객체 조회를 필요한 시점까지 지연처리하는 것이다.
